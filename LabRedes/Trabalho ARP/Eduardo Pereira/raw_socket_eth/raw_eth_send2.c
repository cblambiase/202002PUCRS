...
int main(int argc, char *argv[])
{
	...
	char dest_mac[] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00}; //broadcast
	...
	/* eth destino */
	memcpy(buffer + frame_len, &target_eth, sizeof(target_eth));
	frame_len += sizeof(target_eth);
	...
	//if (sendto(fd, buffer, frame_len + sizeof(target_ip), 0, (struct sockaddr *) &socket_address, sizeof (struct sockaddr_ll)) < 0) {	
		perror("send");
		close(fd);
		exit(1);
	}
